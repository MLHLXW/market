...
<div class="box">
    ...
    <div class="box1">
        <div>
            ...
            <el-button type="primary" class="myButton2" @click="banUserF">永久封禁</el-button>
            <el-button type="danger" class="myButton2" @click="banUser">封禁30天</el-button></el-button>
        </div>
        <div class="the-head">
            <h3 style="margin-left: 82px;">昵称</h3>
            <h3 style="margin-left: 175px;">手机号</h3>
            <h3 style="margin-left: 120px;">诚信分</h3>
            <h3 style="margin-left: 140px;">总收入</h3>
            <h3 style="margin-left: 120px;">总消费</h3>
            <h3 style="margin-left: 235px;">封禁次数</h3>
        </div>
        <div>
            <ul class="MyUl">
                <li  v-for="(item,index) in List" :key="index" v-if="index>pageShang&&index<pageXia">
                     ...
                     <p style="left: 70px;">{{item.name}}</p>
                     <p style="left: 325px;">{{item.phone}}</p>
                     <p style="left: 558px;">{{item.integrity}}</p>
                     <p style="left: 770px;">{{item.income}}元</p>
                     <p style="left: 980px;">{{item.consumption}}元</p>
                     <p style="left: 1330px;">{{item.banNumber}}</p>
                </li>
            </ul>
        </div>
        <div class="footer">
...
export default {
    ...
    methods:{
        async goodsList(){
            const {data: res} = await request.get('/admin/auditUser',{
                headers:{
            ...
            this.List=res;
            this.All_limit=this.List.length;
            console.log(this.List);
        },
        async banUser(){
            for(var i = 0; i < this.List.length; i++){
                if(this.List[i].sign)this.pList.push(this.List[i].phone);
            }
            var temp = JSON.stringify(this.pList+'') //{ indices: false });
            console.log(temp);
            const {data: res} = await request.get('/admin/banUser/temporary',{
                params:{
                    arr: temp,
                },
                headers:{
        ...
        async banUserF(){
            ...
            var temp = JSON.stringify(this.pList+'') //{ indices: false });
            console.log(temp);
            const {data: res} = await request.get('/admin/banUser/permanent',{
                params:{
                    arr: temp,
                },
                headers:{
