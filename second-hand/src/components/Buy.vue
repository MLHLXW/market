...
<div class="box">
    ...
    <div class="box1">
        ...
        <div class="the-head">
            ...
            <h3 style="margin-left: 290px;">商品价格</h3>
            <h3 style="margin-left: 90px;">交易时间</h3>
            <h3 style="margin-left: 80px;">交易状态</h3>
            <h3 style="margin-left: 105px;">操作</h3>
        </div>
        <div>
            <ul class="MyUl">
                <li  v-for="(item,index) in List" :key="index" v-if="index>pageShang&&index<pageXia">
                     <img src='../assets/蟹黄堡1.jpeg'>
                     <p style="left: 325px;">{{item.name}}</p>
                     <p style="left: 735px;">{{item.price}}元</p>
                     <p style="left: 885px;">{{item.trade_time}}</p>
                     <el-button type="primary" disabled class="hos" v-show="pip(item.sellstatus) == 1">交易成功</el-button>
                     <el-button type="success" plain disabled class="hos1" v-show='pip(item.sellstatus) == 2'>挂起</el-button>
...
export default {
    ...
    methods:{
        async goodsList(){
            const {data: res} = await request.get('/user/buyCommodity',{
                headers:{
        ...
        sum(){
            for(var i = 0; i < this.List.length; i++){
                this.sum_math+=this.List[i].price;
            }
        },
        curPage(item){
            this.pageOn = item;
